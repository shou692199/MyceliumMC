...
import MyceliumMC.server.remapper.RemapRules;
import MyceliumMC.server.remapper.RemapUtils;
...
public class MethodHandleMethods {
    ...
    static {
        try {
            MethodHandleMethods.loadMappings(new BufferedReader(new InputStreamReader(MappingLoader.class.getClassLoader().getResourceAsStream("mappings/" + MyceliumMC.getNativeVersion() + "/cb2srg.srg"))));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
