...
public class MyceliumInventoryUtils {
    ...
    public static InventoryHolder getOwner(World world, BlockPos pos) {
        ...
        } else if (state instanceof CraftBlockEntityState) { // MyceliumMC
            TileEntity te = ((CraftBlockEntityState) state).getTileEntity();
            if (te instanceof IInventory)
                return new MyceliumCustomInventory((IInventory) te);
        }
        return null;
    }

    public static Inventory getBukkitInventory(IInventory inventory) {
        InventoryHolder owner = MyceliumInventoryUtils.getOwner(inventory);
        return (owner == null ? new MyceliumCustomInventory(inventory).getInventory() : owner.getInventory());
    }
}
