...
public class MyceliumCustomInventory implements InventoryHolder{
    private final IInventory inventory;
    private final CraftInventory container;

    public MyceliumCustomInventory(IInventory inventory) {
        this.container = new CraftInventory(inventory);
        this.inventory = inventory;
    }

    public MyceliumCustomInventory(ItemStackHandler handler) {
        this.container = new CraftInventoryCustom(this, handler.getStacksList());
        this.inventory = this.container.getInventory();
    }

    public MyceliumCustomInventory(InventoryPlayer playerInventory) {
    ...
    public static InventoryHolder getHolderFromForge(IItemHandler handler) {
        ...
        if (handler instanceof ItemStackHandler) return new MyceliumCustomInventory((ItemStackHandler) handler);
        if (handler instanceof SlotItemHandler) return new MyceliumCustomInventory(((SlotItemHandler) handler).inventory);
        if (handler instanceof InvWrapper) return new MyceliumCustomInventory(((InvWrapper) handler).getInv());
        if (handler instanceof SidedInvWrapper) return new MyceliumCustomInventory((IInventory) ReflectionHelper.getPrivateValue(SidedInvWrapper.class, (SidedInvWrapper) handler, "inv"));
        if (handler instanceof PlayerInvWrapper)  return new MyceliumCustomInventory(getPlayerInv((PlayerInvWrapper) handler));
        return null;
    }
